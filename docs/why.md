...
`go-ruby-rspec/rspec` reimplements Ruby's `rspec` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-rspec/rspec` directly, with no Ruby runtime;
